module OL (Addr(Addr),addrshow,addrread,addrmap,OL(OLTree,OLLeaf),olexample,olshow,olread,olreplace,olmap) where
import Prelude
import Util
...
olreplace addr txt n =
	olmap n $ \a n -> if a/=addr then Nothing
		else case n of
